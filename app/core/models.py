...
from app.core.config import settings, Device
...
def check_device():
    try:
        return Device.CUDA.value if torch.cuda.is_available() else Device.CPU.value
    except Exception:
...
def _determine_inference_device():
    ...
    inference_device = config.inference_device.value
...
async def _cleanup_alignment_cache_whitelist():
    ...
    """
    whitelist = settings.WHISPER_ALIGNMENT.whitelist
    if not whitelist:
        return
...
async def load_align_model_cached(
    ...
    if "multilingual" in settings.WHISPER_ALIGNMENT.models:
        selected_model_name = settings.WHISPER_ALIGNMENT.models["multilingual"]
        logger.info(f"Overriding with 'multilingual' alignment model: {selected_model_name}")
    elif language_code in settings.WHISPER_ALIGNMENT.models:
        selected_model_name = settings.WHISPER_ALIGNMENT.models[language_code]
    ...
    if (selected_model_name is not None
        and selected_model_name == settings.WHISPER_ALIGNMENT.models.get("multilingual")):
        cache_key = "multilingual"
    else:
        cache_key = language_code

    logger.debug(f"settings.WHISPER_ALIGNMENT.models = {settings.WHISPER_ALIGNMENT.models}")
    ...
    if not settings.WHISPER_ALIGNMENT.cache:
        async with alignment_cache_mod_lock:
...
async def load_diarize_model_cached(model_name: str):
    ...
    async def _init_diarization():
        ...
        return whisperx_diarize.DiarizationPipeline(model_name=model_name, device=inference_device, use_auth_token=settings.HF_TOKEN)
    ...
    if not settings.WHISPER_DIARIZATION.cache:
